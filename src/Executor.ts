// Executor — the running instance of a Provider.
//
// A Provider is a description (schemas + resolvers + layers). An Executor is what you get
// when you materialize that description into a runtime: an app-scoped ManagedRuntime is built
// once (holding pools, connections, config), and every `execute` call spins up a per-request
// Context (auth, loaders, per-request state) inside a fresh Scope. Scope finalizes after the
// operation. Resolvers run on the app runtime with the request context provided.
//
...
/**
 * Materialize a Provider into a running Executor. The app Layer is built once; a per-request
 * Context is built from the request Layer on every `execute` call.
 *
 * Call `.dispose()` when the executor is no longer needed (releases the ManagedRuntime and
 * any app-scoped resources).
 */
